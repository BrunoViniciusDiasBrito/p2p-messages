...
describe('group use cases', () => {
  it('creates groups through ports', async () => {
    ...
    const result = await new CreateGroupUseCase(groups, crypto, new NoopDomainEventBus(), new FixedIds()).execute({ ownerPeerId: 'pc_ownerpeer1234567', name: 'Core Team' });
  ...
  it('creates outbound invitations only for accepted contacts', async () => {
    ...
    const group = Group.create({ id: GroupId.create('grp_existing'), name: GroupName.create('Core Team'), ownerPeerId: PeerId.create('pc_ownerpeer1234567'), createdAt: new Date() }, 'evt_1');
    await groups.save(group);
    await contacts.save(Contact.accepted(PeerId.create('pc_inviteepeer12345'), new Date()));
    const result = await new InvitePeerToGroupUseCase(groups, new Invitations(), contacts, crypto, ids).execute({ groupId: 'grp_existing', inviterPeerId: 'pc_ownerpeer1234567', inviteePeerId: 'pc_inviteepeer12345' });
    expect(result.ok).toBe(true);
  });
  ...
  it('rotates epoch when removing a member', async () => {
    const groups = new Groups();
    const group = Group.create({ id: GroupId.create('grp_existing'), name: GroupName.create('Core Team'), ownerPeerId: PeerId.create('pc_ownerpeer1234567'), createdAt: new Date() }, 'evt_1');
    group.addMember(PeerId.create('pc_memberpeer123456'), new Date(), 'evt_2');
    await groups.save(group);
    const result = await new RemoveGroupMemberUseCase(groups, crypto, new NoopDomainEventBus(), new FixedIds()).execute({ groupId: 'grp_existing', peerId: 'pc_memberpeer123456' });
