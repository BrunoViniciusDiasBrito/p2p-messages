...
class Runtime implements PeerNodeRuntimePort {
  ...
  async discoverPeers() { return [{ peerId: 'pc_remotepeer123456', reachability: 'peer_reachable' as const }]; }
...
describe('p2p use cases', () => {
  it('starts peer nodes and discovers peers through runtime ports', async () => {
    ...
    const started = await new StartPeerNodeUseCase(runtime, new NoopDomainEventBus(), ids).execute({ localPeerId: 'pc_localpeer123456' });
    expect(started.ok).toBe(true);
    const peers = await new DiscoverPeersUseCase(runtime, new NoopDomainEventBus(), ids).execute({ localPeerId: 'pc_localpeer123456' });
    expect(peers.ok && peers.value[0]?.peerId).toBe('pc_remotepeer123456');
  });

  it('publishes non-empty envelopes', async () => {
    const published = await new PublishEnvelopeUseCase(new Runtime()).execute({ toPeerId: 'pc_remotepeer123456', envelopeJson: '{"ok":true}' });
